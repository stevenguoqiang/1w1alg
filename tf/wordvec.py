...
def generate_batch(batch_size, num_skips, skip_window):
    ...
    for i in range(batch_size // num_skips):
        target = skip_window
        targets_to_avoid = [skip_window] #避免中心词汇
        for j in range(num_skips):
            while target in targets_to_avoid: #保证中心词对外围词顺序的随机性
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        if step % 2000 ==0:
            if step > 0:
                average_loss /=2000
            print("Avarage loss at step ", step, ":", average_loss)
            average_loss = 0
        if step % 10000 == 0:
            sim = similarity.eval()
            for i in range(valid_size):
                ...
                print(log_str)
    final_embeddins = normalized_embeddings.eval()
